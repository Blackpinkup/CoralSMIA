...
class AssymetricDecoder(nn.Module):
    ...
    def forward(self, feat_list: list):
        ...
        inner_feat_list_ = []
        target_h, target_w = feat_list[0].shape[-2:]
        for idx, block in enumerate(self.blocks):
            feat = inner_feat_list[idx]
            if feat.shape[-2:] != (target_h, target_w):
                feat = F.interpolate(feat, size=(target_h, target_w), mode="bilinear", align_corners=False)
            inner_feat_list_.append(feat)
        
        out_feat = sum(inner_feat_list_) / 4.
        return out_feat
...
def conv_with_kaiming_uniform(use_bn=False, use_relu=False):
    def make_conv(
        ...
        return ConvBlock(in_channels, out_channels, kernel_size, stride,
                         ...
                         dilation=dilation, bias=True, bn=use_bn, relu=use_relu,
                         init_fn=init_conv)
...
class FPN(nn.Module):
    ...
    def forward(self, x):
        ...
        last_inner = getattr(self, self.inner_blocks[-1])(x[-1])
        results = [getattr(self, self.layer_blocks[-1])(last_inner)]
        for feature, inner_block, layer_block, scale in zip(
                x[:-1][::-1], self.inner_blocks[:-1][::-1], self.layer_blocks[:-1][::-1], self.scales
        ):
            if not inner_block:
                continue
            inner_top_down = F.interpolate(last_inner, scale_factor=scale, mode="nearest")
            inner_lateral = getattr(self, inner_block)(feature)
            last_inner = inner_lateral + inner_top_down
            results.insert(0, getattr(self, layer_block)(last_inner))
